...
import mods.modularmachinery.UpgradeStackBuilder;

MachineBuilder.clearScriptDefinitions();

MachineBuilder.newBuilder("crafttweaker:demo_alloy_furnace", "CraftTweaker Demo Alloy Furnace")
...
RecipeAdapterBuilder.newBuilder("demo_furnace_adapter", "crafttweaker:demo_alloy_furnace", "minecraft:furnace")
    .addDurationModifier(0, 40)
    .addItemInputModifier(1, 3.0)
    .addItemOutputModifier(1, 3.0)
    .build();

RecipeBuilder.newBuilder("crafttweaker:demo_alloy_furnace_modularium", "crafttweaker:demo_alloy_furnace", 80)
    .addEnergyPerTickInput(20)
    .addItemInput("minecraft:iron_ingot", 1)
    .addItemInput("minecraft:gold_ingot", 1)
    .addIngredientArrayInput(["minecraft:coal", "minecraft:charcoal"], 1)
    .addCatalystInput("minecraft:redstone", 1).setChance(0.5)
    .addFluidPerTickInput("minecraft:water", 10)
    .addItemOutput("modularmachinery:itemmodularium", 1)
    .addStartCommand("say MMCE CraftTweaker recipe started")
    .addProcessingCommand("say MMCE CraftTweaker recipe ticking", 20)
    .addFinishCommand("say MMCE CraftTweaker recipe finished")
    .build();

RecipeBuilder.newBuilder("crafttweaker:demo_repair_tool", "crafttweaker:demo_alloy_furnace", 40)
    .addEnergyPerTickInput(10)
    .addItemInput("minecraft:iron_pickaxe", 1).consumeDurability(8)
    .addItemInput("minecraft:redstone", 1).setNbt("{\"custom_model\":1}").setTriggerTime(20)
    .addRandomItemOutput(["minecraft:iron_nugget", "minecraft:redstone"], 1).setChance(0.75).setIgnoreOutputCheck(true)
    .build();

val outputUpgrade = UpgradeStackBuilder.newBuilder("minecraft:paper", 1)
    .addModifier("item", "output", 1, 2.0, false, true)
    .build();

// The legacy 1.12 API surface below is still being ported. The 1.21.1 NeoForge
// bridge currently supports basic machine and recipe registration plus raw JSON
// injection through RecipeBuilder.registerMachineJson/registerRecipeJson/registerAdapterJson.
