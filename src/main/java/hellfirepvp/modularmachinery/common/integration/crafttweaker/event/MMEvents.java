...
import github.kasuminova.mmce.common.event.client.ControllerButtonClickEvent;
...
public class MMEvents {
    ...
    @ZenMethod
    public static void onControllerButtonClick(String machineRegistryName, IEventHandler<ControllerButtonClickEvent> function) {
        WAIT_FOR_MODIFY.add(() -> {
            DynamicMachine machine = MachineRegistry.getRegistry().getMachine(new ResourceLocation(ModularMachinery.MODID, machineRegistryName));
            if (machine != null) {
                machine.addMachineEventHandler(ControllerButtonClickEvent.class, function);
            } else {
                CraftTweakerAPI.logError("Could not find machine `" + machineRegistryName + "`!");
            }
        });
    ...
    @ZenMethod
    public static ControllerButtonClickEvent castToControllerButtonClickEvent(MachineEvent event) {
        return event instanceof ControllerButtonClickEvent ? (ControllerButtonClickEvent) event : null;
    }
