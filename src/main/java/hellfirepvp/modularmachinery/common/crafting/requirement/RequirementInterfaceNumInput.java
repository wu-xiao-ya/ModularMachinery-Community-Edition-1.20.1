...
public class RequirementInterfaceNumInput extends ComponentRequirement<Float, RequirementTypeInterfaceNumInput> {
    ...
    public boolean isValidComponent(ProcessingComponent<?> component, RecipeCraftingContext ctx) {
        ...
        return cmp.getComponentType().equals(ComponentTypesMM.COMPONENT_SMART_INTERFACE)
            && ctx.getMachineController().getSmartInterfaceData(type.getType()) != null;
    }

    @Nonnull
    @Override
    public CraftCheck canStartCrafting(ProcessingComponent<?> component, RecipeCraftingContext context, List<ComponentOutputRestrictor> restrictions) {
        SmartInterfaceData data = context.getMachineController().getSmartInterfaceData(type.getType());
        if (data == null) {
