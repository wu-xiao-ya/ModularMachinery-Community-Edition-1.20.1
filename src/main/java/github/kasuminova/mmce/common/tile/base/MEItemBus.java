...
//    protected final BitSet changedSlots = new BitSet();
    ...
    protected synchronized void ensureSlotTrackingCapacity(final int requiredSlots) {
        if (requiredSlots <= 0) {
            return;
        }
    ...
    protected synchronized int[] getNeedUpdateSlots() {
        ensureSlotTrackingCapacity(inventory.getSlots());
        ...
        int bound = Math.min(inventory.getSlots(), Math.min(changedSlots.length, failureCounter.length));
    ...
    public void readInventoryNBT(final NBTTagCompound tag) {
        ...
        ensureSlotTrackingCapacity(this.inventory.getSlots());
